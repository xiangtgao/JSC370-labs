...
institution <- stringr::str_extract_all(
  publications_txt,
  "(University of [:alpha:]+(?: [:alpha:]+){0,2} | [:alpha:]+(?: [:alpha:]+){0,2} Institute of [:alpha:]+(?: [:alpha:]+){0,2})"
  ) 
...
schools_and_deps <- stringr::str_extract_all(
  publications_txt,
  "(School of [:alpha:]+(?: [:alpha:]+){0,2} | Department of [:alpha:]+(?: [:alpha:]+){0,2})"
  )
...
-   189 articles without abstracts
...
titles <- stringr::str_remove_all(titles, "^[\\s]+|[\\s]+$")
```
...
-   None of them are without titles
...
table <- knitr::kable(database)
kableExtra::scroll_box(table, width = "800px", height = "500px")
